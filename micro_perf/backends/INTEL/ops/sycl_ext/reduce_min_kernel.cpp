  ...
  for (int b = 0; b < batch; ++b) {
    for (int d = 0; d < dim_size; ++d) {
      ...
      if (input_mode == "sin") {
        ...
        throw std::runtime_error("Unsupported input_mode for reduce_min_sycl.so: " + input_mode);
      }
...
template <typename scalar_t>
py::dict reduce_min_benchmark_impl(
    int64_t batch_i64,
  ...
  if (warmup != 0) {
    throw std::runtime_error("warmup is not supported in binary-equivalent mode for reduce_min_sycl.so");
  }
  ...
  if (!d_in || !d_min || !d_idx) {
    throw std::runtime_error("Failed to allocate SYCL buffers in reduce_min_sycl.so");
  }
  ...
  if (verify) {
    ...
    for (int b = 0; b < batch; ++b) {
      ...
      for (int d = 0; d < dim_size; ++d) {
        ...
        if (!keep_lhs_min_or_nan(ref_min, ref_idx, v, d)) {
          ref_min = v;
          ref_idx = d;
        }
      }

      float got_min = to_float(h_min[b]);
      max_abs_err = std::max(
          max_abs_err,
          std::abs(static_cast<double>(got_min) - static_cast<double>(ref_min)));
      if (h_idx[b] != ref_idx) {
        ++mismatch;
      }
    }
...
py::dict reduce_min_benchmark(
  ...
  if (dtype_str == "float32") {
    return reduce_min_benchmark_impl<float>(
        batch, dim_size, iterations, verify, input_mode, warmup, smalldim_mode, rows_per_group);
  }
  if (dtype_str == "float16") {
    return reduce_min_benchmark_impl<sycl::half>(
        batch, dim_size, iterations, verify, input_mode, warmup, smalldim_mode, rows_per_group);
  }
  if (dtype_str == "bfloat16") {
    return reduce_min_benchmark_impl<sycl::ext::oneapi::bfloat16>(
        batch, dim_size, iterations, verify, input_mode, warmup, smalldim_mode, rows_per_group);
  }
  throw std::runtime_error("Unsupported dtype for reduce_min_sycl.so: " + dtype_str);
}

template <typename scalar_t>
py::dict reduce_min_compute_impl(
    int64_t batch_i64,
    int64_t dim_size_i64,
    const std::string& input_mode,
    const std::string& smalldim_mode,
...
py::dict reduce_min_compute(
  ...
  if (dtype_str == "float32") {
    return reduce_min_compute_impl<float>(
        batch, dim_size, input_mode, smalldim_mode, rows_per_group);
  }
  if (dtype_str == "float16") {
    return reduce_min_compute_impl<sycl::half>(
        batch, dim_size, input_mode, smalldim_mode, rows_per_group);
  }
  if (dtype_str == "bfloat16") {
    return reduce_min_compute_impl<sycl::ext::oneapi::bfloat16>(
        batch, dim_size, input_mode, smalldim_mode, rows_per_group);
  }
  throw std::runtime_error("Unsupported dtype for reduce_min_sycl.so: " + dtype_str);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def(
      "reduce_min_benchmark",
      &reduce_min_benchmark,
      "ReduceMin benchmark via in-process SYCL extension",
      py::arg("batch"),
      py::arg("dim_size"),
      py::arg("dtype"),
  ...
  m.def(
      "reduce_min_compute",
      &reduce_min_compute,
      "Compute reduce_min values/indices once and return tensors for strict correctness checks",
      py::arg("batch"),
      py::arg("dim_size"),
      py::arg("dtype"),
      py::arg("input_mode") = "sin",
